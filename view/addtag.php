...
### API_METHODS: youtube.videos.list, youtube.videos.update
/**
...
if ($client->getAccessToken()) {
  try{
    ...
    if (empty($videoList)) {
      ...
      $newtagString = $_POST["newtag"];
      $newtags = explode(",", $newtagString);
      if (is_null($tags)) {
        $tags =  $newtags;
      } else {
    ...
    $htmlBody .= sprintf('<li>Tags "%s" added for video %s (%s) </li>',
        implode(',',$responseTags), $videoId, $updateResponse['snippet']['title']);
